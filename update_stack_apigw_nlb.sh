...
aws cloudformation update-stack --stack-name apigw-nlb --template-body file://./cf/apigw_nlb.yaml \
    --parameters ParameterKey=Subnets,ParameterValue=\"$SUBNET_IDS\" \
                 ParameterKey=VPC,ParameterValue=$DEFAULT_VPC_ID

aws cloudformation wait stack-update-complete --stack-name apigw-nlb
