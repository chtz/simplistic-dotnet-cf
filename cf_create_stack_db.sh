...
aws cloudformation create-stack --profile admin-hslu --stack-name mydb --template-body file://./cf/database.yml \
    --parameters    ParameterKey=Subnets,ParameterValue=\"$SUBNET_IDS\" \
                    ParameterKey=VPC,ParameterValue=$DEFAULT_VPC_ID

aws cloudformation wait stack-create-complete --profile admin-hslu --stack-name mydb
